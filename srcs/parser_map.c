...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/07 20:42:39 by acabiac           #+#    #+#             */
/*   Updated: 2021/04/07 20:42:42 by acabiac          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_around_cell(int x, int y, t_cub *cub)
{
	...
	if (x > 0 && !ft_strchr("021 NEWS", cub->map[y][x - 1]))
		return (return_error("Error\nInvalid character in map\n", -1));
	if (y < cub->map_h - 1 && !ft_strchr("021 NEWS", cub->map[y + 1][x]))
		return (return_error("Error\nInvalid character in map\n", -1));
	if (y > 0 && !ft_strchr("021 NEWS", cub->map[y - 1][x]))
		return (return_error("Error\nInvalid character in map\n", -1));
...
int		ft_check_map_cell(int x, int y, t_cub *cub, int *spawn)
{
	...
	else if (ft_isalpha(cub->map[y][x]))
	{
		...
		*spawn = 1;
		...
		return (set_sprite_position(x, y, cub));
	return (0);
}
